...
- **Triggers (`/triggers`)** — List with enable/disable toggle (`PATCH`). ADMIN create form is discriminated: CRON (cron expression + scene) or SENSOR (sensor device + state + scene). See [automation.md](automation.md) for firing semantics.
